...
const singleBackwardMaxScrollConfigList = configList.map(config => ({
  ...config,
  custom: {
    ...config.custom,
...
const massForwardScrollsConfigList = configList.map((config, index) => ({
  ...config,
  custom: {
    direction: Direction.backward,
...
const massBackwardScrollsConfigList = massForwardScrollsConfigList.map(
  (config, index) => ({
    ...config,
    custom: {
      direction: Direction.backward,
...
const massBouncingScrollsConfigList_fwd = massForwardScrollsConfigList.map(
  (config, index) => ({
    ...config,
    custom: {
...
const massBouncingScrollsConfigList_bwd = massForwardScrollsConfigList.map(
  (config, index) => ({
    ...config,
    custom: {
      direction: Direction.backward,
...
const getInitialItemsCounter = (misc: Misc): ItemsCounter => {
  ...
  const oppositeItem = misc.scroller.buffer.getEdgeVisibleItem(
    Direction.backward
  );
...
const getCurrentItemsCounter = (
  ...
  const itemsToFetch =
    previous.direction && previous.direction !== direction
      ...
      : singleFetchCount;
  const newDirIndex =
    previous.get(direction).index + // previous edge index
...
const shouldScroll: ItFuncConfig<ICustom> = config => misc => done => {
  ...
  spyOn(misc.workflow, 'finalize').and.callFake(() => {
    ...
    if (cycles === 1) {
      ...
      itemsCounter = getCurrentItemsCounter(
        misc,
        custom.direction,
        itemsCounter
      );
    ...
    if (cycles < wfCount) {
      ...
      expect(misc.padding[direction].getSize()).toEqual(
        itemsCounter.get(direction).padding
      );
      expect(misc.padding[opposite].getSize()).toEqual(
...
const _makeTest = (data: MakeTestConfig<ICustom>) =>
  makeTest({
    ...data,
