...
const checkPromisifiedMethod: ItFuncConfig<ICustom> =
  config => misc => done => {
    const { workflow } = misc;
    ...
    misc.adapter.isLoading$
      ...
      .subscribe(() => {
        ...
        const method = misc.adapter[token] as unknown as (
          opt: unknown
...
const checkConcurrentSequences: ItFuncConfig<ICustom2> =
...
describe('Adapter Promises Spec', () => {
  ...
  describe('Call before init', () =>
    [Adapter.relax, Adapter.reload, Adapter.reset, Adapter.check].forEach(
      token =>
        it(`should resolve immediately ("${token}")`, async () => {
          const misc = new Misc(configureTestBedSub());
          const method = misc.adapter[
            token
          ] as unknown as () => Promise<AdapterMethodResult>;
