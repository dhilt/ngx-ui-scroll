...
const shouldVirtualize =
  ...
  async done => {
    ...
    expect(scrollableSizeAfterInsert).toEqual(
      scrollableSize + amount * itemSize
    );
    expect(scrollableSizeAfterInsert).toEqual(
  ...
  [Operation.append, Operation.prepend].forEach(token =>
    [false, true].forEach(opposite => {
      ...
      [false, true].forEach(virtualizeWhenEof =>
        makeTest({
          config: (virtualizeWhenEof ? configVirtualWhenEOF : configVirtual)[
            token
          ],
          title:
