...
const shouldPrependMany: ItFuncConfig = () => misc => async done => {
  ...
  const itemsToPrepend = generateItems(
    NEW_ITEMS_COUNT,
    indexToPrepend - NEW_ITEMS_COUNT
  ).reverse();
...
const shouldPrependVirtual =
  (config: TestBedConfig, increase: boolean): ItFunc =>
  misc =>
  async done => {
    const {
      datasource,
    ...
    expect(viewport.paddings.backward.size).toEqual(
      bwdPadding + amount * itemSize
    );
