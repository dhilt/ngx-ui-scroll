...
const configList: TestBedConfig<ICustom>[] = [{
  ...
  datasourceName: 'limited-51-200-no-delay',
  datasourceSettings: {
    startIndex: 51, bufferSize: 20, padding: 0.2, windowViewport: true, minIndex: 51, maxIndex: 200, adapter: true
  },
...
const expectations = (config: TestBedConfig<ICustom>, misc: Misc, done: () => void) => {
...
const preExpectations = (config: TestBedConfig<ICustom>, misc: Misc, done: () => void) => {
  ...
  const runExpectations = () => {
    ...
    const startIndex = config.datasourceSettings.startIndex as number;
    const edgeIndex = startIndex + (position === 0 ? 0 : config.custom.items - 1);
