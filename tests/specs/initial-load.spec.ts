...
const getSetItemSizeCounter = (
  settings: TestBedConfig,
  misc: Misc,
  itemSize: number
): ItemsCounter => {
...
const testSetItemSizeCase: ItFuncConfig = settings => misc => done =>
  misc.scroller.state.cycle.busy.on(pending => {
    ...
    const itemSize = tplSettings[
      misc.horizontal ? 'itemWidth' : 'itemHeight'
    ] as number;
...
const testNotSetItemSizeCase: ItFuncConfig = settings => misc => done =>
  misc.scroller.state.cycle.innerLoop.busy.on(loopPending => {
    ...
    if (loopCount === 1) {
      ...
      itemsCounter = getNotSetItemSizeCounter(
        settings,
        misc,
        itemSize,
...
const testLackOfItemsOnFirstFetchCase: ItFuncConfig =
  config => misc => async done => {
    const unsubscribe = misc.scroller.state.cycle.innerLoop.busy.on(
      loopPending => {
        if (loopPending) {
          return;
        }
        ...
        expect(misc.adapter.firstVisible.$index).toEqual(
