...
const configList: TestBedConfig<ICustom>[] = [
  ...
  ...conf,
  datasourceClass: getDatasourceClassForInsert({
    settings: conf.datasourceSettings
  })
}));

const configListByIndexes = [configList[0], configList[3]].map(c => ({
  ...c,
  custom: {
    ...c.custom,
...
const configDynamicList = [
  ...configList,
  configDecreaseList[0],
  configDecreaseList[1]
];
...
const baseConfigVirtual = {
  datasourceSettings: {
    // [-1, 0, 1, 2, 3] are visible after init
    minIndex: -4,
    maxIndex: 5,
    startIndex: 0,
    padding: 0.1,
...
const configListVirtual: TestBedConfig<ICustomVirtual>[] = [
  {
    ...
    custom: {
      ...
      result: {
        min: -4,
        max: 7,
        list: [-4, -3, MAX + 1, MAX + 2, -2, -1, 0, 1, 2, 3, 4, 5]
      }
    }
  },
  {
  ...
  },
  {
    ...
    custom: {
      ...
      result: {
        min: -4,
        max: 7,
        list: [-4, MAX + 1, MAX + 2, -3, -2, -1, 0, 1, 2, 3, 4, 5]
      }
    }
  },
  {
  ...
  },
  {
    ...
    custom: {
      ...
      result: {
        min: -4,
        max: 7,
        list: [-4, -3, -2, -1, 0, 1, 2, 3, 4, MAX + 1, MAX + 2, 5]
      }
    }
  },
  {
  ...
  },
  {
    ...
    custom: {
      ...
      result: {
        min: -4,
        max: 7,
        list: [-4, -3, -2, -1, 0, 1, 2, 3, MAX + 1, MAX + 2, 4, 5]
      }
    }
  },
  {
...
const configListEmpty: TestBedConfig<ICustomEmpty>[] = [
  ...
  datasourceClass: getDatasourceClassForInsert({
    settings: conf.datasourceSettings,
    common: {
      // set impossible limits to emulate empty datasource
      limits: { min: 1, max: -1 }
    }
  })
}));
...
const doCheck = (
  ...
  if (!dynamic) {
    expect(newData.bufferSize).toEqual(
      prevData.bufferSize + (shouldInsert ? amount : 0)
    );
  }
  expect(newData.viewportSize).toEqual(
    prevData.viewportSize + (shouldInsert ? amount * ITEM_SIZE : 0)
  );
  if (!shouldInsert) {
    ...
    if (!dynamic) {
      expect(newData.bufferedIndexes).toEqual(
        Array.from(
          Array(newData.bufferSize),
          (_, i) => prevData.bufferedIndexes[0] - (decrease ? amount : 0) + i
        )
      );
...
const shouldCheckInBufferStatic =
  ...
  async done => {
    ...
    ds.insert(
      items,
      index,
      before ? Direction.backward : Direction.forward,
      decrease
    );
...
const shouldCheckInBufferDynamic =
  (config: TestBedConfig<ICustom>, shouldInsert: boolean): ItFunc =>
  misc =>
  done => {
...
const shouldCheckVirtual: ItFuncConfig<ICustomVirtual> =
...
const shouldCheckEmpty: ItFuncConfig<ICustomEmpty> =
  config => misc => async done => {
    await misc.relaxNext();
    const { before, decrease, amount, index, result } = config.custom;
    const {
