...
const configVirtualWhenEOF: OperationConfig<Operation, ICustom> = {
  [Operation.prepend]: {
    datasourceSettings: { ...basicDSConfig, startIndex: min },
    datasourceClass: getDatasourceClassForReset({
      ...basicDSConfig,
...
const shouldVirtualize =
  (operation: Operation, fixOpposite: boolean, virtualizeOnEof: boolean): ItFunc =>
    misc =>
      async done => {
        await misc.relaxNext();
        const {
        ...
        const {
          custom: { amount }
        } = (virtualizeOnEof ? configVirtualWhenEOF : configVirtual)[operation];
        const isAppend = operation === Operation.append;
        const direction = isAppend ? Direction.forward : Direction.backward;
        ...
        const scrollableSize = viewport.getScrollableSize()
        const oppositePaddingSize = paddings[oppositeDirection].size;
        ...
        const _lastVisibleId = misc.adapter.lastVisible.data.id;

        expect(scrollableSize).toEqual((max - min + 1) * itemSize);
        expect(virtualizeOnEof ? paddingSize : oppositePaddingSize).toEqual(0);
        ...
        if (!isAppend) {
          await misc.adapter.prepend({
            items,
            increase: fixOpposite,
            ...(virtualizeOnEof ? { virtualize: true } : { bof: true })
          });
          expect(buffer.firstIndex).toEqual(_firstIndex + shift);
        } else {
          await misc.adapter.append({
            items,
            decrease: fixOpposite,
            ...(virtualizeOnEof ? { virtualize: true } : { eof: true })
          });
        ...
        const scrollableSizeAfterInsert = viewport.getScrollableSize();
        expect(scrollableSizeAfterInsert).toEqual(scrollableSize + amount * itemSize);
        expect(scrollableSizeAfterInsert).toEqual((maxIndex - minIndex + 1) * itemSize);
        expect(paddings[direction].size).toEqual(paddingSize + amount * itemSize);
        expect(paddings[oppositeDirection].size).toEqual(oppositePaddingSize);
        ...
        expect(viewport.getScrollableSize()).toEqual(scrollableSizeAfterInsert);
...
describe('Adapter Append-Prepend Spec', () =>
  [Operation.append, Operation.prepend].forEach(token =>
    [false, true].forEach(opposite => {
      ...
      [false, true].forEach(virtualizeWhenEof =>
        makeTest({
          config: (virtualizeWhenEof ? configVirtualWhenEOF : configVirtual)[token],
          title:
            `should ${token} virtually after scroll` +
            (virtualizeWhenEof ? ' (even if EOF/BOF)' : '') +
            (opposite ? ' (opposite)' : ''),
          it: shouldVirtualize(token, opposite, virtualizeWhenEof)
        })
      );
