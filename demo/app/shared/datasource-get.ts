...
export const datasourceGetObservableInfinite =
  (demoContext: DemoContext) =>
  (index: number, count: number): Observable<MyItem[]> =>
    new Observable<MyItem[]>((observer: Observer<MyItem[]>) =>
      observer.next(datasourceGetInfinite(demoContext, index, count))
    );

export const datasourceGetPromiseInfinite =
  (demoContext: DemoContext) =>
  (index: number, count: number): Promise<MyItem[]> =>
    new Promise<MyItem[]>(success =>
      success(datasourceGetInfinite(demoContext, index, count))
    );

export const datasourceGetCallbackInfinite =
  (demoContext: DemoContext, delay?: number): IDatasource['get'] =>
  (index, count, success) =>
    delayedCall(
      datasourceGetInfinite(demoContext, index, count),
      success,
      delay
    );
...
export const datasourceGetCallbackLimited =
  ...
  (index, count, success) =>
    delayedCall(
      datasourceGetLimited(demoContext, min, max, index, count),
      success,
      delay
    );
